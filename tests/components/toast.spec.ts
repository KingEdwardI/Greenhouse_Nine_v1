...
test.describe("Toast Component", () => {
  ...
  test("shows success toast when success button is clicked", async ({ page }) => {
    await page.goto("/?story=overlays---toast--basic");
    await page.waitForTimeout(1000);
  ...
  test("shows warning toast when warning button is clicked", async ({ page }) => {
    await page.goto("/?story=overlays---toast--basic");
    await page.waitForTimeout(1000);
    ...
    await expect(page.getByText("Heads up").first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test("shows error toast when error button is clicked", async ({ page }) => {
    ...
    await expect(page.getByText(/Something went wrong/).first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test("shows info toast when info button is clicked", async ({ page }) => {
    ...
    // Wait for toast to appear
    await expect(page.getByText(/informational/).first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test("renders glass toast trigger buttons", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: "Success (Glass)" })).toBeVisible();
    await expect(page.getByRole("button", { name: "Warning (Glass)" })).toBeVisible();
    await expect(page.getByRole("button", { name: "With Action (Glass)" })).toBeVisible();
  });
  ...
  test("shows glass success toast when clicked", async ({ page }) => {
    ...
    // Wait for toast to appear
    await expect(page.getByText(/glass effect/).first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test("shows glass toast with action button", async ({ page }) => {
    ...
    // Wait for toast to appear
    await expect(page.getByText(/pending items/).first()).toBeVisible({ timeout: 10000 });
  });
