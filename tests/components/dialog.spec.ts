...
test.describe("Dialog Component", () => {
  test("renders dialog trigger and opens on click", async ({ page }) => {
    ...
    await expect(page.getByText("A short description provides helpful context.")).toBeVisible();
    await expect(page.getByText("Dialog body content goes here.")).toBeVisible();
  ...
  test("closes dialog when cancel is clicked", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Dialog Title" })).not.toBeVisible();
  });
  ...
  test("renders glass variant dialog", async ({ page }) => {
    ...
    const triggerButton = page.getByRole("button", { name: "Open Glass Dialog" });
    await expect(triggerButton).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Glass Dialog" })).toBeVisible();
    await expect(page.getByText("A dialog with glass morphism effect.")).toBeVisible();
    await expect(page.getByText(/This dialog demonstrates the glass morphism effect/)).toBeVisible();
  });
