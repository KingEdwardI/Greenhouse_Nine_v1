...
function parseDiff(diff: string): DiffLine[] {
  ...
  for (const line of lines) {
    if (line.startsWith('+++') || line.startsWith('---')) {
      ...
      parsed.push({ type: 'added', content: line.slice(1), lineNumber: contextLineNum });
      contextLineNum++;
    } else if (line.startsWith('-')) {
      parsed.push({ type: 'removed', content: line.slice(1), lineNumber: contextLineNum });
      // Don't increment for removed lines
    } else if (line.startsWith(' ')) {
      parsed.push({ type: 'context', content: line.slice(1), lineNumber: contextLineNum });
      contextLineNum++;
    } else if (line.trim()) {
      parsed.push({ type: 'context', content: line, lineNumber: contextLineNum });
      contextLineNum++;
    }
  }
...
export function DiffViewer({ diff, fileName, maxHeight = 300 }: DiffViewerProps) {
